       ...
       # print("ser.in_waiting",ser.in_waiting)
        if ser.in_waiting>0 :
             ...
             if read_signal== b'A': ## equivalent to A
                 vb_list=np.load('vb1.npy')
                 ub_list=np.load('ub1.npy')
             ...
             #### calculate tan theta
                 ...
                 ret,green_binary=cv2.threshold(G,5,255,cv2.THRESH_BINARY)  ### 7
...
################################################################# FIND Z ########################################################
                 ...
                 if all(delta<10 for delta in deltau):
                     deltau=[0]*len(deltau)
                 ...
                 #for i in range(a,b-110):    
                  #  cv2.circle(image,(uo_list[i],vo_list[i]),1,(0,255,0),-1)       ### Object Pixels
                 for i in range(len(uo_list)):    
                    cv2.circle(image,(uo_list[i],vo_list[i]),1,(0,255,0),-1)       ### Object Pixels
                 #for i in range(a,b):    
                  #  cv2.circle(image,(uo_list[i],vo_list[i]),1,(0,255,0),-1)       ### Object Pixel
                 for i in range(len(vb)):    
                    cv2.circle(image,(ub[i],vb[i]),1,(0,0,255),-1)                    ## Baseline
                 ####### Baseline VS Object pixels saving Image ########################
                 
                 image_path = os.path.join('/home/pi/Desktop/saved_images/final_results/Images_of_new_method/laser up/baselina_and_object', f'image_{counter}.png')
